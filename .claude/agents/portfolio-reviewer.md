...
The canonical live portfolio state is **`output/html/portfolio.html`**, regenerated by `python3 src/refresh_portfolio.py --write`. It joins:
...
2. Read `output/html/portfolio.html` for CMP, P&L, target, upside %, action label
   - If portfolio.html is older than ~7 days, run `python3 src/refresh_portfolio.py --write` first
