...
**The single source of truth for portfolio state is `output/html/portfolio.html`**, auto-generated (2026-07-20) by `venv/bin/python3 scripts/build_portfolio_page.py`. It combines:
- `data/portfolio.csv` — qty + avg buy price (manual; refresh after trades)
- fresh **bhavcopy** close — CMP (official end-of-day, fetched at build time)
- each stock's `research/SYMBOL.md` — action label (via the shared parser)
...
1. **Get current holdings:**
   ...
   - If portfolio.html looks stale, run `venv/bin/python3 scripts/build_portfolio_page.py` first (it refetches the bhavcopy close). Prices are always fresh; holdings are only as current as portfolio.csv.
