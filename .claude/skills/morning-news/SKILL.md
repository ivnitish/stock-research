...
8. **If full mode:**
   ...
   b. **Telegram daily message — threads, taught (format decided 2026-07-18, depth rule relaxed same day).** If the `TELEGRAM_BOT_TOKEN` env var is set, POST ONE plain-text message to chat ID **1679797853**. Teach every macro thread that *genuinely matters* to the portfolio that day — 1 on quiet days, 2-3 on busy days — each with the full Feynman treatment: every causal link spelled out in plain language, no jargon without explanation, ending in what it means for the user's holdings. **Depth is non-negotiable, count is flexible:** never compress a thread into a headline to fit more in — if it earns a slot it gets the full chain; if not, it goes in the side notes.
      ...
      **Titles (user feedback 2026-07-18):** state the mechanism plainly ("Oil & the rupee", "US rates and FII flows"), never editorial hooks ("the story just flipped" — banned style, see .claude/rules/writing-quality.md). Draft the digest prose and the MACRO_THREAD entry to the `no-ai-slop` house style from the first sentence (`.claude/skills/no-ai-slop/SKILL.md`) — it is the writing standard, not a post-edit; the sourced headline-bullet sections of MORNING_BRIEF.md are exempt. The content bar is the Feynman treatment itself; a thread that teaches a mechanism well earns its slot even if the immediate portfolio conclusion is "no action yet". Budget: up to ~3,800 chars, but always ONE message — never chunk. Format:
   ...
   c. **Per-stock news snapshots (event-driven, added 2026-07-18).** If any *covered* holding (has a `research/SYMBOL.md`) had genuinely hard news today — quarterly results, large order/contract, regulatory action, management change, dilution/fundraise — run the `stock-snapshot` skill for it: a structured Investment Snapshot (verdict, financials, drivers, risks, takeaway) delivered as its own Telegram message, news event as the lead line. **Max 1 snapshot per day** (reduced from 2 on 2026-08-16, holding the daily ceiling at two messages) — the single hardest news item only; everything else stays a line in the theme digest. Days without hard news send the theme digest alone — the "one daily message" baseline still holds; snapshots fire only on real events.

      **Delivery.** If the `SNAPSHOT_OUT` env var is set, write the composed snapshot text to that path and do **not** send it yourself — a deterministic later step delivers it, and an absent file means "no hard news today". This is how the GitHub Actions run works, and it is why a dropped snapshot there turns the build red instead of vanishing silently. If `SNAPSHOT_OUT` is unset (local cron, interactive sessions), send directly as before via `venv/bin/python3 scripts/send_session_takeaways.py --stdin`.
