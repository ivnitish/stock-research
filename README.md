...
- `com.nitish.stocks.daily-news` → `scripts/daily_news_cron.sh` — weekdays 08:42 IST with retry slots at 11:42/14:42/17:42/20:42 (each run no-ops once the day's brief succeeds — stamp file `data/logs/daily_news_last_success`; lock dir prevents overlap). Headless Claude run: morning news brief for holdings (Telegram ping + GitHub issue) + buy-at alert check (web-sourced CMPs, no broker MCPs). Log: `data/logs/daily_news.log`. Once the claude.ai cloud routine (see below) is live, these local slots become the fallback: each run first checks GitHub for today's `Morning News YYYY-MM-DD` issue and stands down if the cloud already delivered.
- `com.nitish.stocks.fintwitter-finds` → `scripts/daily_fintwitter_cron.sh` — weekdays 09:15 and 18:15 IST (retry slot; stamp `data/logs/fintwitter_finds_last_success`). Headless Claude run: scans Indian fintwitter / ValuePickr / ThreadReader for stock ideas (contrarian lens in `docs/FINTWITTER_WATCHLIST.md`), writes `docs/FINTWITTER_FINDS.md`, sends the plain-text `## Telegram summary` section to your bot via `scripts/send_telegram_digest.py`. Log: `data/logs/fintwitter_finds.log`. On-demand: `python3 scripts/fintwitter_finds.py` (dry-run) or `--full`.
