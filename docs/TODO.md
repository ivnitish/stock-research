...
- **Verdict: do not subscribe at ₹938-988.** Post-issue market cap ₹6,348 Cr on 64,253,069 shares = **57.8x FY26 PAT of ₹109.90 Cr**, 28.5x EV/EBITDA, 4.9x post-issue book. MOS **−42%**, asymmetry **0.03x** against a 1x floor.
- **Post-listing ladder (corrected against the CLAUDE.md decision matrix after an advisor check).** Primary path: **BUY REDUCED 3-5% at ₹555**, conditional on the CDMO contracts being quantified in a quarterly print, which re-grades C → B (MOS 20.4%, asymmetry 2.44x = the Grade A/B 2-3x row). Fallback: **SPECULATIVE 1% max at ₹418, stop ₹330, never scales**. My first draft said "BUY AT ₹418, initiate 5%" — Grade C has no BUY row at all, and Phase 0.5 independently caps a thesis resting on unbuilt capability at 1-2%. Both routes converge on 1%.
- **Anchor book is the counter-case and is stated at full strength, not footnoted.** ₹526.20 Cr from 34 investors at ₹988 (the cap) on 21-Aug, 5,325,909 shares; **60.06% domestic mutual funds** across 11 houses / 19 schemes including HDFC Pharma & Healthcare, Mirae Asset Health Care, ICICI Pru Smallcap, Motilal Oswal Small Cap; insurers 7.98%; Citigroup Mauritius, BNP Paribas ODI, Singularity Equity Fund I alongside. Professional money reached the opposite conclusion on the same prospectus. Limits noted: 30/90-day anchor lock-in is shorter than the FY28 ramp it implies faith in, and a filled book measures demand at a price, not return from it.
...
- **Reframed as house style, not post-edit (user directive, same session):** the standard governs the writing from the first draft; detect mode is verification only. Wired into ALL composed-prose surfaces: `stock-research` (rule 6 rewritten "WRITE to the house style"), `morning-news` (theme digest + MACRO_THREAD entry; sourced headline-bullet sections of MORNING_BRIEF.md exempt), `stock-snapshot` (Rules), `fintwitter-finds` (Rules), `writing-quality.md` (retitled "House writing standard + voice pass"), and a House-usage note at the top of the skill itself.
- **Updated KRSNAA PDF sent to Telegram** (msg 142, `output/pdf/KRSNAA_2026-07-25.pdf`) — caption flags prose-only change, verdict unchanged.
